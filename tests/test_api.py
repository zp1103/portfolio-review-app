...
class ApiTests(unittest.TestCase):
    ...
    def test_analysis_page_separates_active_and_objective_products(self) -> None:
        ...
        self.assertIn("扣除资金进出后的组合表现", response.text)
        self.assertIn("1000.00", response.text)
        self.assertIn("2026-06-26", response.text)
        self.assertIn("2026-08-21", response.text)
        self.assertIn('class="chart-axis-label"', response.text)
        self.assertIn('class="chart-marker chart-marker-start"', response.text)
        self.assertIn('class="chart-marker chart-marker-latest"', response.text)
